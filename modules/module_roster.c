...
int module_open(struct song_entry *data)
{
	...
	snprintf(fd_table[fd].data, 1024, template, data->fields[0],
		data->fields[1], data->fields[2], data->fields[3], data->fields[4]);
	fd_table[fd].filesize = strlen(fd_table[fd].data);
